...
2. **Use Copilot to debug**: Open `tests/pages/LoginPage.ts` -- it contains 3 intentional bugs. Instead of trying to fix them yourself, paste each error message into Copilot Chat and ask for help:
...
4. **Generate new test cases**: Pick 3 scenarios from your Stage 1 test plan that cover the login flow. For each one, describe it to Copilot in plain English and ask it to write the test. For example:
