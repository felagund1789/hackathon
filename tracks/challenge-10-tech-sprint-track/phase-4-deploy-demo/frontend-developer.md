# Phase 4: Ship and Demo -- Frontend Developer Tasks

[Back to Phase 4 Overview](../phase-4-deploy-demo.md) | [Back to Technical Team Sprint Track](../../challenge-10-tech-sprint-track.md)

**Time: ~1 hour tasks + demo/retro**

## Tasks

1. **Fix critical UI bugs** -- Address any issues from QA's final pass.

2. **Visual polish** -- Spacing, color consistency, loading states, empty states. Small details that make the demo look professional.

3. **Demo contribution** -- Walk through the UI: component structure, responsive behavior, how Copilot helped with component generation and styling. Consider leading the live walkthrough section of the demo.

## Verification

- [ ] No critical UI bugs remaining
- [ ] Visual polish applied
- [ ] Demo contribution prepared

---

Previous: [Phase 3 -- Frontend Developer Tasks](../phase-3-sprint-2-integration/frontend-developer.md) | Next: [Phase 5 -- Frontend Developer Tasks](../phase-5-agentic-workflows/frontend-developer.md)
