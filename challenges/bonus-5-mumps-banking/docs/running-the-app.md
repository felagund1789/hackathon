...
The devcontainer at `.devcontainer/bonus-5-mumps-banking/` handles setup automatically. It runs `setup-yottadb.sh` on creation, which installs the [YottaDB](https://yottadb.com/) open-source MUMPS implementation.
...
Run these from the `challenges/bonus-5-mumps-banking/` directory. The path must be absolute -- relative paths break if you change directories after setting `ydb_routines`. This sets the variable from scratch rather than appending to it, which avoids accumulating stale paths across multiple exports.
...
**Installation fails** -- The devcontainer setup script treats YottaDB install failure as non-fatal (`|| true`). If it failed during container build, run `bash .devcontainer/bonus-5-mumps-banking/setup-yottadb.sh` manually. Check that you are on a supported architecture (amd64 or aarch64).
